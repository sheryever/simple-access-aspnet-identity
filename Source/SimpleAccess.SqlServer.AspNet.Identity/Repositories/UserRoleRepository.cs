    ...
    public class UserRoleRepository<TUser> where TUser:IdentityUser
    {
        ...
        public void Insert(TUser user, string roleName)
        {
            using (var conn = new SqlConnection(_connectionString))
            {
                ...
                var idObject = SqlHelper.ExecuteScalar(conn, CommandType.Text,
                ...
                if (!string.IsNullOrEmpty(roleId))
                {
                    ...
                    using (var conn1 = new SqlConnection(_connectionString))
                    {
                        ...
                        SqlHelper.ExecuteNonQuery(conn1, @"Insert into aspnetuserroles(UserId,RoleId) VALUES(@Id,@RoleId)", parameters1);
                    }
                }
            }
        }
            ...
            using (var conn = new SqlConnection(_connectionString))
            {
                ...
                if (!string.IsNullOrEmpty(roleId))
                {
                    ...
                    using (var conn1 = new SqlConnection(_connectionString))
                    {
                        ...
                        SqlHelper.ExecuteNonQuery(conn1, @"Delete FROM aspnetuserroles WHERE UserId=@Id AND RoleId=@RoleId", parameters1);
                    }
                }
            }
        }
